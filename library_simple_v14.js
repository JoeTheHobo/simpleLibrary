...
Array.prototype.each = function(type,response) {
    ...
    if (typeof type == "object" && type.length == undefined) whatToPerform = "style";
    if (typeof type == "object" && type.length != undefined) whatToPerform = "multiAction";
    ...
    for (let i = 0; i < this.length; i++) {
        ...
        if (whatToPerform == 'style') {
            this[i].css(type);
        }
        if (whatToPerform == 'innerHTML') {
            this[i].innerHTML = response;
        }
        if (whatToPerform == 'placeholder') {
            this[i].placeholder = response;
        }
        if (whatToPerform == 'src') {
            this[i].src = response;
        }
        if (whatToPerform == 'id') {
            this[i].id = response;
        }
        if (whatToPerform == 'class') {
            this[i].class = response;
        }
        if (whatToPerform == 'classAdd') {
            this[i].classList.add(response)
        }
        if (whatToPerform == 'classRemove') {
            this[i].classList.remove(response)
        }
        if (whatToPerform == 'function') {
            type(this[i]);
        }
        if (whatToPerform == 'create') {
            typeCreateDomReturnList.push(this[i].id.create(response));
        }
    }
...
sloglibrary(15,'Simple','JoeTheHobo');