...
function _type(ele) {
    ...
    if (returnObj.type == "number") {
        ...
        returnObj.isEven = ele % 2 == 0;
        returnObj.isOdd = ele % 2 !== 0;
        returnObj.isPositive = ele > 0;
        returnObj.isNegative = ele < 0;
        returnObj.isWhole = ((ele*10) % 10) == 0;
    ...
    if (returnObj.type == "string") {
        ...
        returnObj.strictChracterCount = _type_countCharactersInString(ele);
        returnObj.chracterCount = _type_countCharactersInString(ele.toLowerCase());
        returnObj.alphabetCount = _type_countCharactersInString(ele.toLowerCase().format("[allow\\abcdefghijklmnopqrstuvwxyz]"));
    }
...
function _type_countCharactersInString(string) {
    let chars = {};
...
String.prototype.format = function(format,condition) {
    ...
    let inCommand = false;
    let allowedCharacters = false;
    for (let i = 0; i < format.length + 1; i++) {
        let char = format.charAt(i);
        //Catch if no number
        if (gettingNumber && isNaN(char)) {
            index = numberIndex;
    ...
    if (allowedCharacters) {
        ...
        for (let i = 0; i < finalString.length;i++) {
            if (allowedCharacters.includes(finalString.charAt(i)))  newString += finalString.charAt(i);
        }
...
sloglibrary(16.3,'Simple','John Jones');
